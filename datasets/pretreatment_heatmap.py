...
class HeatmapAlignment():
    ...
    def center_crop(self, heatmap):
        ...
        if self.align:
            ...
            raw_heatmap = raw_heatmap[y_top - self.offset: y_btm + 1 + self.offset,
                          (self.heatmap_image_size // 2) - (height // 2): (self.heatmap_image_size // 2) + (
                                      height // 2) + 1]
...
class TransferDataset(Dataset):
    def __init__(self, args, generate_heatemap_cfgs) -> None:
        ...
        assert self.dataset_name.lower() in ["sustech1k", "grew", "ccpg", "oumvlp", "ou-mvlp", "gait3d", "casiab",
                                             "casiae"], f"Invalid dataset name: {self.dataset_name}"
...
def get_args():
    ...
    parser.add_argument('--heatemap_cfg_path', type=str, default='configs/skeletongait/pretreatment_heatmap.yaml',
